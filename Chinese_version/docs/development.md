...
- 在 Windows 上写码，在装有 NMRPipe/SMILE 的 Linux 机器上做真实引擎验证。
...
- 测试与临时产物不要进仓库：pytest 用 `--basetemp`（或系统临时目录）；
  `scripts/vm_test.sh` 在临时目录里跑，并把字节码/ruff 缓存一并重定向过去；
- 需要长期保留的脚本放仓库 `scripts/`；一次性排查脚本用完即删；
- Python 环境用项目自己的 venv（首次 `python main.py` 自动创建）。

**测试夹具纪律(强制,2026-09-16,Phase 12)**:测试**不得**依赖只在某一台机器上存在的
绝对路径文件。这类依赖不会报错,只会让整组用例在别的机器/CI 上被 `skipif` 静默跳过,
等于没有测试。需要真实文件布局时,在
...
改任何文件前先保证状态**可恢复**:进行中的改动先 `git stash create` 取一个 stash 提交并
打 `backup/<日期>-<主题>` 标签(或用 `git bundle create` 把完整仓库归档到仓库之外),
备份失败就不继续改。恢复用 ``git checkout backup/<标签> -- <文件>`` 或从 bundle 克隆。
...
## 行为指纹与变更分级(强制,2026-09-20)

改动 `core/`、`backend/`、`workflow/`、`nmrforge_api/` 或随包数据
(`nmrforge_data/config/nmrforge.yaml`、`nmrforge_data/presets/`)后:**先判断分级,再更新声明**,否则
`tests/test_compat.py` 会红(守卫「静默改行为」):

```bash
python scripts/update_compat_declaration.py --level additive          # 只新增入口/字段
python scripts/update_compat_declaration.py --level behavior_changed \
    --affected localization,sweep_detection --note "改了什么"        # 数值会变
python scripts/update_compat_declaration.py --level same              # 只是注释/文案
```

- 分级:`same`(代码 token 不变)/ `additive`(下游不用重跑)/ `behavior_changed`
  (**数值会变**,必须给 `affected`)/ `contract_changed`(列/字段/错误码变了,同步
  下游契约镜像与守卫);
- 声明在 `nmrforge_api/compat_declaration.py`(纯数据模块,刻意排除在指纹之外;
  不要往里写逻辑);
- 指纹口径:`behavior_digest` 覆盖四棵代码树 + 随包数据的内容(用户本地配置
  `nmrforge_data/config/nmrforge.local.yaml` 不算);`token_digest` 去注释/docstring,中英两个语言
  副本在无代码差异时相等;
- 产物行为戳 `run.json` / `records/reference.json` / `records/manifest.json` 由
  `nmrforge_api.compat.record_stamp()` 统一写入,不要各自拼;
- 黄金向量 `python -m nmrforge_api compat --golden` 必须与声明一致(改了行为就同步
  声明里的 `golden`)。

## 记录/状态写盘:一律原子替换(强制,2026-09-20)

- 凡是**整份写出、整份读回**的 JSON(研究状态、`run.json`、`workflow.json`、
  参考/记录 JSON、定位附件、兼容清单导出、逐 step 相位与 NUS 参数缓存、
  `*.quality.json`、SMILE 排名/报告)一律走 `core.project.manager.atomic_write_text`
  (同目录临时文件 + `os.replace`),不要直接 `Path.write_text`:写盘中途被中断
  (崩溃/断电/被并发读者撞上)时,读者看到的要么是完整的旧版本、要么是完整的新版本。
- 追加型日志(`run.log`、`qc_audit.jsonl`、CSV 流)不受此限。
- 改造这类位置时**只换写盘机制**,序列化表达式原样保留 —— 文件字节不变,下游指纹、
  缓存 fp、黄金向量都不该动(守卫:`tests/test_compat.py`、黄金向量)。
...
- 窗向量必须与 NMRPipe 逐点一致(0.2.191):SP 首点乘 -c(SP 缺省 0.5,
  GM/EM 缺省 1.0);GM 高斯常数 k=1/(2*sqrt(ln2))=0.6005612(实测,
