...
- **v0.11.0 同时发布源码与 AppImage。** AppImage 由 PyInstaller 打成，`nmrforge_data/config`、`nmrforge_data/presets`、`gui/assets`、`ui_support/locales` 通过 spec 的 `datas` 进 `_MEIPASS`（数据包保持同形），资源定位见 `core/app_paths.py` 与 `ui_support/i18n.py`。
