...
> **后续(2026-09-23 补注)**:这组「简单/进阶」显示层相位途径在 0.2.106 被统一相位方案取代 ——
> 显示层相位引擎、NUS 混合流程优化器及其配套测试都已从树里删除,当前实现见
> `workflow/phase_routes.py` 与统一相位方案。本页保留为当时的设计记录:下文提到的模块、
> 入口与测试**都不随当前版本发布**。
> 日期:2026-08-16。
...
- 显示层相位引擎与 NUS 混合流程优化器当年的配套测试(希尔伯特逐维相位恢复、近零相位、
  基线阶数、窄峰填零建议;混合流程调用顺序、直接维 override、间接维候选选择、后端次数):
  随机制一并下线,现不在树内。
...
- ±180° 绝对符号消歧(0.2.106):presets/*.yaml 新增 peak_sign_regions
  (化学位移分区+期望符号,HNCACB 13C 轴 Cα 40-70ppm 负/Cβ 15-45ppm 正)。
  对 mixed 实验搜索轴所在核有分区先验时,统计区内强峰占优符号:两区相反
  且各自干净(≥70%/≥4 强峰)而绝对约定与预设不符才 p0+=180 翻转(保守)。
  绝对符号随脉冲序列/处理可能翻转,预设可整体取反。sampleB F1≈0 与约定
  一致,不翻转。
