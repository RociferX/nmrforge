...
`--rebuild-peak-tables` 只重算两张参考峰表(用已有的冻结参考谱 + `reference.list`,不动谱与
...
| `--localize-peaks` | **限定峰的定位**:只精修 CSV(至少含 `peak_id` 列)里的峰;检出、行数、`peak_id` 编号不变,未列入的峰保留(位置取检出阶段抛物线);缺省 = 全谱。CSV 可带 `condition` 列(**多条件研究**:每个条件只取自己的行,缺行报错;一个文件即可服务 A/B) |
| `--localize-peaks-gaussian` / `--localize-peaks-parabolic` | **逐方法**限定(覆盖 `--localize-peaks`):常用「parabolic 全谱 + 只对真值峰做 gaussian」;同样支持 `condition` 列 |
