...
| [`packaging/`](../packaging/README.md) | AppImage 构建素材(v0.11.0 已发布) |
...
v0.11.0 提供两样东西:**Linux AppImage**与**源码**。AppImage 自带 Python 与 Qt,
...
- [真实数据实测证据(匿名)](docs/evidence/real-data-comparison.md) —— 自动谱 vs 手工谱的对比图、
  维护者的人工核验结论、按默认阈值选峰的匹配率、QC 评分、两套处理参数的并排表,以及真机重复性
...
(含 LGPL-3.0)随产物分发。两个 v0.11.0 产物都完成了维护者私有仓库里的发布检查清单
(许可正文与声明在产物内、可替换/重链接、干净机器验收、记录 SHA-256)。这不改变 nmrForge
自身源码的 Apache-2.0 条款。
...
- **从源码使用 nmrforge**:Apache-2.0,包括专利授权、保留署名声明的要求,以及再分发修改过的
  文件时说明改动的要求;
- **AppImage**:两个 v0.11.0 产物已完成上述验收;再分发前按同一清单复核;
