"""Linux 负载诊断:监控 CPU/内存/负载/uptime,并可选施加有界 CPU+内存负载。

用途(用户,2026-09-09):排查「高负载导致关机/断电」。本机是 VM 宿主,客户机内
看不到宿主温度(/sys/class/thermal 只有 cooling_device,无温度读数),所以 VM 突然
重启/关机=宿主端动作(温度/电源/资源)而非客户机自身电源事件。

用法示例:
  例:python scripts/diag_load_monitor.py --load cpu --csv /tmp/diag.csv --duration 30
  例:python scripts/diag_load_monitor.py --load cpu --csv /tmp/diag.csv --duration 30
"""
