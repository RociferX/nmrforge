...
直接维相位搜索原语（供 backend 与 unified 相位路线使用）：
- ``search_direct_spectrum_phase``：直接维 FT 谱 (p0, p1) 聚合搜索；
- ``nus_direct_phase``：NUS 直接维非均匀 DFT 相位校正；
- ``search_direct_phase_on_spectrum``：重构平面/终谱上对称性评分搜索。
（0.2.164 起删除 search_phase/search_spectrum_phase/apply_phase_axis 等
被 memory_phase_search 取代的旧入口，旧 git 历史保留。）
