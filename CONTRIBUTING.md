...
3. **State backward compatibility.** If a change alters the meaning of a parameter, a study
   ...
   changes to the `nmrforge_api` surface must be recorded in
   [docs/external-api/CHANGELOG.md](docs/external-api/CHANGELOG.md) and
...
The project also has an internal workflow for its own maintainers (an NMRPipe validation host,
ownership checks, and a single long-lived `master` branch). That is documented in
