...
- 默认 `gaussian_roi_max_points: 0`(不限制)→ 与旧版结果一致:真机 4× Δ≤1e-06 ppm、
  两维 2× Δ=0、零方法翻转;提速 1.38×(1×)/1.64×(2×)/1.65×(4×)/1.59×(两维 2×);
- 设正值(如 48)仍是可选提速项(4× 1.73×、两维 2× 1.84×),但 4× 上最大改 0.39 ppm
  (3 个峰翻转)、两维 2× 最大 0.71 ppm(1 个翻转);触发逐峰留档 `roi_capped`,
  截断后拟合失败自动用完整 ROI 重试(`fit_retry_uncapped`/`retry_n_iter`);
- 配置解析允许 0(0 = 不限制);文档 05/07 与真机对照表同步。

## 0.2.7(2026-09-14)

**逐峰 2D 高斯拟合提速**(用户实测:组合 sweep 里 zero_fill 翻倍后单峰 0.031 s →
0.816 s,run 17-22 s → 5-9 min)。

- 解析式雅可比:同一次迭代只做 1 次残差求值(数值差分约 7 次);数学等价,
  实测峰位差 ≤3e-05 ppm、方法零翻转;
- 拟合窗口每轴半宽上限 `peaks.localization.gaussian_roi_max_points`(默认 48 点):
  细网格下限制拟合规模;触发时记录 `roi_capped`/`roi_half_points`;
- `gaussian_max_nfev`(默认 400→200)可配;解析雅可比下 200 求值 ≈ 200 迭代
  (原数值差分 ~57 迭代),最坏成本减半;
- 真机实测(248 峰跟踪,同一数据):1× 2.04→1.52 s(-25%)、2× 3.93→2.28 s(-42%);
  峰位/回退计数不变;VM 全量 1076 passed / 19 skipped。

## 0.2.6(2026-09-14)

**逐维指定参数**(用户:「参数组合表会对两个维度分别指定吗」)。

- 组合表/网格的点号键本来就逐维生效(`window.F1.off`、`baseline.F2.enabled`、
  `linewidth_hz.F1`、`phase.<轴>.…`);本次补两处:
- **填零逐轴裸标量 = 倍数**:`zero_fill.F1=2` 现在是 2×TD(与全局
  `zero_fill=2` 同义);此前被当成「显式 SI=2」。显式 SI 用
  `zero_fill.F1.size=512` / `{"mode": "size", "size": 512}`;
- **目标数字分辨率可逐轴**:`points_per_line` 接受 `{"F1": 2.0, "F2": 4.0}`
  (`resolve_points_per_line` 透传映射;逐轴取值与回退在 `zero_fill_plan`);
- 文档新增「05-inputs §5.9 按维指定参数」表;测试 +4(逐轴填零倍数/显式
  size/关闭、逐轴 points_per_line、组合表逐维键端到端)。

## 0.2.5(2026-09-14)

- **组合表空单元格 = 未指定**(沿用参考基底):CSV 里某参数列留空 / YAML-JSON 里
  写 `null` 时不再当成「覆盖成空值」(真机发现:直接把基值清空并标成 `source=combo`;
  与「留空即不覆盖」的直觉相反)。
