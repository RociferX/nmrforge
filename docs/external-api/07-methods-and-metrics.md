...
| 拟合窗口每轴半宽上限(`peaks.localization.gaussian_roi_max_points`) | 48 点 | 只在该轴 ppm-ROI 换算点数 **> 48** 时生效(比 1× 再细约 2 倍以上);触发时写进留档 `roi_capped`/`roi_half_points`,窄峰实测中心/宽度无变化,只有“点宽接近上限”的极宽峰可能被截断(可调大或关闭) |
