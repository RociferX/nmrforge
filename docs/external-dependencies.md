...
installation is vendored under `scripts/` or `presets/`. See
...
Default configuration lives in `config/nmrforge.yaml`. Copy it to
`config/nmrforge.local.yaml` for machine-local overrides; that file is git-ignored precisely so
