...
- 导入/数据组间分析下拉必须是独立顶层窗口(0.2.194):Wayland 用 Popup
  (先设 transientParent 再 show),Windows/X11 用无边框 Tool 窗口(先
  show 再 move,全局坐标按屏幕可用区域定位)——不要退回主窗口子部件方案
  (0.2.163-补4),Windows 上首次弹出会被中央部件盖住。
