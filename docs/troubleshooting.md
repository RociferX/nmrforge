...
2. Point nmrForge at it explicitly in `config/nmrforge.yaml` (or `config/nmrforge.local.yaml`):
...
## "本机未找到 tcsh/csh (NMRPipe scripts need a C-shell)"
...
A related message is `未找到 proj3D.tcl (NMRPipe projection tool)` - the NMRPipe installation is
...
4. If the framework reduced the direct-dimension zero filling to fit memory, that is reported:
   "内存不足:直接维填零已降为 1×TD 以降低 SMILE 内存". Prefer making the reduction explicit so it
...
path is locked down (an ACL-restricted sandbox, or a stale `pytest-of-<user>` directory left by an
earlier interrupted run), pytest cannot create its base temp directory.
