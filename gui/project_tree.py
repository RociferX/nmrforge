...
层级(契约 v1.2 §8.5):
- 项目节点:右键删除项目(强确认);
- 实验节点:可空白创建,右键导入样品数据/重命名/删除;
- 样品数据节点:实验下的样品数据条目(可多组),右键生成 FID/生成谱图/打开目录/删除。

Backend 落地 DataEntry 层级后直接使用 entry.data;当前兼容阶段
(schema 1.1)以实验自身作为单样品数据节点。
"""
...
class ProjectTreePanel(QWidget):
    ...
    def current_experiment_id(self) -> str:
        """返回当前选中节点所属实验 id(选择样品数据节点时归一化到实验)。"""
    ...
    def _request_group_add(self, exp_id: str, group_id: str) -> None:
        ...
        if not candidates:
            ...
            InfoDialog(self, "没有可加入的数据", "实验下没有未入组的样品数据").exec()
            return
    ...
    def _folder_path_for_item(self, item: QTreeWidgetItem) -> Path | None:
        ...
        # data 节点(0.2.199-补29ge/补29gf,用户):双击/右键「打开所在目录」
        # 与「在终端中打开」都用 d_xxx 数据基座;raw 子节点自身可开终端。
        try:
    ...
    def begin_create_experiment(self, initial: str = "") -> None:
        """新建空白实验:项目树内内联命名(不弹窗),回车提交 / Esc 取消。"""
        if self.manager.project is None:
            return
