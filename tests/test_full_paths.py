"""Full path end-to-end regression test (0.2.163-patch8). This file must be run after each code
modification: complete simulation goes through all paths -- automatic (2D/3D uniform + NUS),
manual (fid.com/ spectrum script), batch (data group). Strategy: FakeBackend simulates NMRPipe
product; spectrum reading is injected into the synthetic array through monkeypatch to verify
process integrity, product homing, WorkflowRun Registration, optimisation/real execution of
diagnostic phase (fid in readable format, diagnostic/direct dimension window optimisation to
read real file; baseline/zero filling/preview in injection array)."""
...
def _synthetic_preview(axis: int, p0: float) -> np.ndarray:
    """Complex Lorentzian: complex of the evaluated axis + known phase, other axes real (isomorphic
    to phase_routes test)."""
    n0, n1 = 96, 80
...
class FakeBackend:
    """Simulate NMRPipe backend: write real readable fid; spectrum path/preview provided via
    monkeypatch."""
    ...
    def convert_to_fid(
        ...
        """Write real readable fid (readable by diagnostic/direct dimension window optimisation)
        and fid.com (read manually)."""
        work = self.work
    ...
    def _write_spectrum_file(path: Path, ndim: int = 2) -> None:
        """Write readable pipe spectrum (real + valid header), For peak selection/Quality score
        reading."""
    ...
    def _write_fid_file(self, path: Path, n_traces: int, n_direct: int) -> None:
        """Write nmrglue readable fid:512B header + (n_traces, n_direct) duplicate interleaved."""
        import nmrglue as ng
        import nmrglue.fileio.pipe as pmod
        ...
        data[3, 20] = 100 + 50j
        data[5, 40] = 80 + 20j
        # Replicate -> Reality/virtual interleaved real (double the first axis), match NMRPipe fid
        # layout.
    ...
    def reconstruct_nus(self, experiment, params, progress=None) -> dict:
        ...
        # Write the reconstruction plane (2D nus2d/recon.ft1;3D nus3d_rc/test*.ft1) for finalize
        # consumption.
        if experiment.ndim >= 3:
...
def _install_spectrum_mocks(monkeypatch: pytest.MonkeyPatch) -> None:
    """Inject synthesis preview/spectrum reading (real process, file reading layer simulation)."""
    ...
    # Baseline / zero filling score: Spectrum file bytes are not readable -> Directly read and
    # synthesize array.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("hnca_3d", "HNCA"),  # 3D uniform(Slice).
        ("nus_3d", "NUS 3D"),  # 3D NUS(Slice).
    ],
)
def test_auto_full_path(
    ...
    """Automatic path: import -> generate FID -> generate spectrum (unified, diagnosis +
    optimisation) -> peak selection. The analysis (HSQC CSP) function has been deleted according
    to user decision (2026-09-12, REPORT-008), and this path ends at peak selection."""
    ...
    # Slice fid existence: single file or slice directory.
...
def test_auto_uniform_runs_processing_optimization(
    ...
    """Uniform processing parameter optimisation: diagnosis + baseline + direct dimension window +
    zero filling/indirect window into final run."""
    ...
    monkeypatch.setattr(
        ...
        lambda work, experiment, current=None: MultiWindowOptimizeResult(
            choice={"F1": {"type": "none"}},
            changed=True,
            logs=["Test indirect dimension window"],
        ),
    )
    spec = generate_spectrum(manager, exp_id, data_id, backend)
    assert spec
...
def test_manual_full_path_uniform(
    ...
    """Artificial path: manual_fid_com -> run_manual_fid_com -> manual_scripts ->
    run_manual_spectrum."""
    ...
    class Runtime:
        def run(self, argv, *, cwd=None, timeout=3600, on_line=None):
            ...
            if name == "fid.com":
                ...
                # Real script output {raw directory name}.ft2 (manual uses read_dataset dataset_id).
    ...
    backend = FakeBackend(manager.data_dir(exp_id, data_id, "process"))
    # 0.2.199-patch29dm: Manually must automatically generate FID(fid.com before reading).
    generate_fid(manager, exp_id, data_id, backend)
...
def test_manual_scripts_uses_data_id_for_single_fid(
    ...
    """0.2.163-patch10: 2D artificial spectrum script in_file uses data_id(d_001) instead of raw
    directory name (src_*), which is consistent with the generate_fid product."""
...
def test_manual_reads_segmented_container(
    ...
    """0.2.163-patch12: Segmented collection container directory (without acqus) manual reading no
    longer reports an error."""
    from workflow.manual import manual_scripts

    # Construct a segmented container: root directory without acqus, two subsections containing
    # acqus.
    ...
    # Manual_scripts Priority is given to existing script; if it can be read, acqus error will not
    # be reported.
...
def test_manual_scripts_slice_in_file(
    ...
    """0.2.163-patch9: After the 3D uniform manual spectrum script detects the slice fid, the
    rendering in_file is rewritten as fid/test%03d.fid (manual operation will not fail)."""
    from workflow.manual import manual_scripts
...
def test_manual_spectrum_accepts_slice_fid(
    ...
    """Artificial spectrum operation: 3D slice fid(fid/test*.fid) is not misjudged as missing
    fid."""
...
def test_batch_full_path_group(
    ...
    """Batch path: group import -> data group batch processing (refer to data processing +
    optimisation in sequence)."""
