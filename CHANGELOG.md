...
- 直接维相位搜索继续优化(Backend):
  - 候选并行回退:真实运行 python 无响应(疑似线程池与 numpy/BLAS 嵌套并行
    死锁),已回退串行并记录;
  - 窗口指标向量化:内窗批处理 + 边缘窗标量,仅旋转窗口切片而非整个 3D
    数组——结果逐位一致,VM 实测 cc 89→26s(3.4×)、sampleB 20→2s(10×);
  - 叠加 phase.json 缓存(0.2.127):首次后重复生成谱图直接跳过搜索;
- 测试:本地全量 595 passed、ruff 全绿;VM 全量 591 passed + 4 skipped
  (Python 3.12.13,HEAD 328eba3)。
