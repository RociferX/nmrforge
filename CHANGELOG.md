...
- 生成谱图步骤重新处理拆两按钮:「重新优化」(完整自动处理,相位/参数
  ...
  仍为「重新处理」;
- 关键保证:重新运行终脚本不再直接执行写死旧 EXT 窗口的 .com 文件,
  而是提取最近成功 WorkflowRun 的 effective params(相位/窗/基线)
  合并当前 final_ext_lo/hi 覆盖,经 generate_spectrum(phase_route=none)
  重新渲染脚本——用户改了直接维范围再重跑即生效;
