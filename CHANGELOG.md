...
- 测试:test_viewer_1d 新增 3 例 + 扩展条带联动用例(鼠标映射/条带对齐/
  峰对齐/点击选中),全量 415 passed,ruff 全绿。
...
- UI 调整(用户反馈):
  ...
  - viewer 控制面板紧凑化(小间距/矮滑块/图层列表限高/条带更小);
  - 主窗口默认高度降至 720,底部 Task/Log 限高 160(展开不挤出三栏);
  - 右侧谱图面板加宽(初始 260/460/560);
  - 批量导入自动检查子文件夹:添加总文件夹时扫描其下所有含 acqus 的
    数据集目录并加入列表;「单个导入」与「批量处理」改为两个分组框,
    明确是两种可选导入方式;
  - 打开项目改为单击(左侧树项目节点 + 欢迎页最近项目),无需双击;
  - 1D 条带顶部加高(110px),主界面中完整显示;
  - Pipeline 状态按当前选中数据计算(不再总是显示首个数据的聚合状态);
- 生成谱图/FID 阶段进展日志:ProcessingController.generate_fid /
