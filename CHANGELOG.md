...
- 层级命名:实验类型 → 数据类型(菜单/项目树/中间页/运行历史/日志等用户可见
  文案统一);内部数据模型仍为 experiment,Shared Contract 不变。
- 注释字段调整:数据类型注释仅保留「实验类型」字段,取值指认实验 / 动力学
  实验;原实验类型注释的「维度 / 实验类型(presets)/ 核」三个字段移到样品
  数据注释(与 重复/Buffer 组分/Buffer pH/温度 并列)。
- 导入自动填充同步改为填充样品数据注释(维度/实验类型/核/温度),数据类型
  注释不再由导入自动填充。
- 测试:字段 schema、注释读写回读、中间注释条、菜单文案、注释表单(数据
  类型指认/动力学选项 + 样品数据先选维度再过滤 presets)、导入自动填充
  位置;本地全量 542 passed + ruff 全绿。
