...
- 补1(用户实测定位):首次打开时下拉跑位到页面顶部附近(「实验类型」标签
  处,只露出滚动条一半)——根因是隐藏状态 move 的位置解释在 Windows 上
  不可靠(0.2.162-补14 同款问题),且下拉 isVisible 后重复点击只置顶不
  重定位,所以「怎么点都没有」;改为先 show 再 move,首次打开即到位;
  「数据组间分析」要点两次才弹出同样因此修复。
