...
- **用户可见错误信息(Phase 21,CLI 出口)**:`python -m nmrforge_api` 不再把路径写错、YAML
  ...
  (`docs/tasks/2026-09-17-phase21-error-messages.md`)。
...
- **参考冻结后阈值锁定**:再给与参考不同的阈值 → `ReferenceError`(CLI 退出码
  2),不悄悄重选峰——后续所有参数扰动只能沿用参考的阈值;与参考一致则复用;
  想换阈值须重建参考(`force=True` 或删该条件的 `study/reference/<key>/`);
- 阈值留档 `peak_params.sigma_multiplier` / `previous_sigma_multiplier` /
  `detection.sigma_multiplier` / `threshold_source`;每条 workflow 记录
...
- 以 API 引入前提交 `2670108` 为基线复核本体改动;确认本体不依赖
  ...
  清单等改动边界成立;
- 修复 SMILE 候选边距仍按采集参数估算的问题:优先从每张实际候选 FT2 头读取
  处理后 ppm 轴,避免 EXT/裁剪/轴映射变化导致峰计数与排名漂移;
- 修复 SMILE 边距日志在候选评估前生成、始终误报“未评估”的时序错误;
- 修复切换数据时阈值信号可能把上一数据的峰定位方法写入当前数据;
- 修复 GUI 人工覆盖 `.list` 后遗留旧 `.list.localization.json` 的产物不一致;
- 新增 3 个回归场景;相关专项 `66 passed`,路径/所有权测试 `16 passed`,本地全量
  `1071 passed, 1 skipped`,`ruff check .` 全绿;
- VM `9c8ffeb` 全量 `1053 passed, 19 skipped`;真实 uniform API 4/4 workflow
...
- `nmrforge_api` 扫描口径细化为「人工可调参数」并新增两类入口:
  1) **相位偏差轴** `phase_delta.<轴>.p0|p1`(相对参考相位,±5° 之类)与
     `phase.<轴>.p0|p1`(绝对值):在参考相位上施加后传给候选运行;直接写
     `phases`/`direct_phase` 现在会报错并提示改用相位偏差轴;
  2) **显式组合表** `combos=`(正交/部分因子/D-optimal/LHS/手挑由外部决定,
     接口原样按表序执行,不做设计决策)+ 表格 I/O(`load_combo_table`/
     `write_combo_table`/`combos_from_rows`)+ 信息性诊断 `design_diagnostics`
     (水平计数/重复行/成对相关最大 |r|/缺失水平);CLI 增加 `sweep --combos`;
  3) 网格键校验:锁定的键报错,确定性/策略参数(提取窗口、目标点距、采样表、
...
- 真机默认路径(不传峰表,软件自动选 152 峰):48.9 s、4/4 组合、152/152 峰位
...
- 实现:`SpectrumPanel` 文件菜单两条(打开当前数据谱图 / 打开任意谱图...);
  新动作 `_on_menu_open_current_spectrum`:无数据上下文 → 「当前没有选中数据」;
  有数据但无谱 → 「当前数据还未生成谱图」;有谱 → 走 `load_current_spectrum()`
  (与 Pipeline「展示谱图」同一函数,含峰表加载);
- 测试:`tests/test_gui_layout.py` 新增正反三例(无数据/无谱/有谱)+ 菜单文案断言;

## 0.2.199-补29hz-修26(2026-09-11,用户):谱图放大后顶部多出一块空白(里面只有「谱图」)
- 现象(用户):谱图查看器点「放大」后,上面莫名多一块空白,里面就「谱图」两个字;
- 根因:放大时把**横向** QSplitter 换进面板的**垂直**布局;横向 splitter 只声明
  「水平可扩展」,垂直剩余高度没有可扩展项 → 全部被压给顶部标题行(VM 实测标题行
  18px → 311px、标题 QLabel 同高,绘图区被挤到 494px);
- 解决:`_enter_expand_mode` 中 `outer.setStretch(indexOf(hsplit), 1)` —— 标题行回到
  sizeHint、绘图区吃满剩余高度(实测 18px / 787px);还原路径同索引仍是拉伸项,行为不变;
- 验证:VM 离屏渲染 + 布局几何诊断;新增数值守卫(放大后标题行 ≤40px、放大 splitter
  ≥ 面板高 −80px),守住「不许再出现空白块」;
...
- 规则(最终口径):
  * **净真峰优先**(默认)→ 候选用**全采样**脚本重建一次:峰计数/质量分/拟合残差即终谱口径;
  * **一致性优先** → 候选用**留出(train)**脚本重建一次:留出点不参与重建,残差作排序依据,
    峰数/质量分来自这次留出重建(该口径下如此,日志会注明);
  两种情况都**每候选只跑一次**(不再是修22 的两次);
- 实现:`scan_smile_parameters` 按 `rank_mode` 决定是否给 `smile_scan` 传 `holdout_ratio`
(净真峰 → 0;一致性 → `SMILE_HOLDOUT_RATIO`);后端候选循环回到单跑(运行哪份脚本由调用方决定),
扫描结束日志写明「扫描运行方式:全采样重建/留出重建」;耗时估算去掉修22 的 ×2;
- 验证:本地 + VM 单测断言「净真峰口径 1 次全采样跑、一致性口径 1 次留出跑、上榜脚本始终全采样」;
- 注:修22 的两跑做法仅存在于本日中间提交,已被本条取代(未进入用户交付口径)。

## 0.2.199-补29hz-修22(2026-09-11,用户):峰计数用全采样重建,留出集只算一致性(已由 修23 取代)
- 用户:「对于峰计数应该用全部点做 smile,一致性才是留出部分」;
- 原状(修5/修10 引入 holdout 时):打开留出就把扫描用的 `-sample` 换成 `nuslist_train`,
  候选谱(峰计数/质量分/拟合残差都取自它)其实是 **75% 采样**重建的,与最终全采样脚本
  不是同一张谱 —— 与「这一步要尽量重构出更多真峰」的判断口径不符;
- 改为(backend.smile_scan,候选循环拆两段):
  ① **全采样重建**(prod 脚本=终跑脚本模板) → 峰计数 / 质量分 / squeeze `smile_rms_ratio`;
  ② **留出重建**(train 脚本) → 只算 `holdout_*` 一致性残差;
  两张候选谱都在评估后删除(候选仍只短暂存在于内存盘);
- 成本与提示:候选耗时≈2×(全采样+留出),`estimate_scan_seconds` 在 holdout>0 时 ×2;
  GUI 排序说明补一句「峰计数来自全采样重建(留出集只用于一致性口径)」;
- 测试:新增 `tests/test_2d_nus_compat.py::test_smile_scan_runs_full_then_holdout` —— 断言
  每候选跑两次 SMILE、第一次不带 `nuslist_train`、第二次带、上榜脚本仍是全采样表;

## 0.2.199-补29hz-修21(2026-09-11,用户):SMILE 优化入口限制为「仅 2D NUS」(3D 暂时隐藏)
- 用户:「把 3D nus 的 smile 优化入口隐藏了,这个功能暂时不太理想」;
- 改动:`PipelinePanel._smile_supported()` 取代原来的「仅 NUS」判定 —— SMILE 步骤行只在
  **2D NUS** 数据显示(3D NUS 与非 NUS 一样隐藏);步骤描述同步改为「仅 2D NUS」;
- 防御:`run_step` 的程序化入口同样按新口径拦(消息「SMILE 优化仅适用于 2D NUS 数据」);
  `ProcessingController.optimize_smile` 增加 ndim 检查(3D 抛「暂仅支持 2D NUS」),
  防止将来从其它入口(批量/脚本)绕过 UI 直接跑;
- 附带效果:「下一步」与首次导入气泡本来就不把 SMILE 当必做步骤(0.2.199-补29as),
  且「可选做:SMILE 优化」提示随行可见性 —— 3D NUS 下不再出现;
- 测试:`tests/test_gui_pipeline_state.py` 既有用例改为显式 2D NUS(fake experiment 带 ndim=2),
  新增 3D NUS 隐藏用例;`tests/test_gui_smile.py` 新增控制器拒绝 3D 的用例;

## 0.2.199-补29hz-修20(2026-09-11,用户):UI 几何守卫(像素级自检,不再靠肉眼)
- 用户追问:「这种问题为什么你无法直接检查到」——当时我只量了「自己改的东西」(下拉框文本余量),
  没有量「这一行的整体几何」;测试套件里也没有任何几何断言,于是「全绿」给了假安全感。  根因是 `setSpacing(6)` **声明了却没用**这类静默失效,靠读代码不容易看出来;
- 新增步骤行几何守卫 `tests/test_gui_step_row_layout.py::test_step_rows_have_no_geometry_problems`:
  6 个步骤行 × 5 种行宽(320/380/440/560/760),自动检查 ① 可见控件两两不重叠;
  ② 同一行的相邻控件间距 ≥ 4 px;③ 控件不超出行宽;
- 守卫当场又查出一个此前没注意的问题:**头部状态图标与标题贴死(0 px)** → 加 4 px 间隔;
- 至此 UI 类问题有三道自动守卫:几何(重叠/间距/越界)、下拉框文本余量(不省略号)、
  文案(选项名与悬停说明的信息量);换行/DPI 差一点就出问题的项都能被自动挡住;
- 经验(写入 problems.md):改 GUI 后必须跑几何守卫;「pytest 全绿」≠「看着顺眼」,
  像素/几何/文案必须各自有断言;看不到图时,先把「该量什么」定义成断言再改界面;

## 0.2.199-补29hz-修19(2026-09-11,用户):步骤行控件挨太紧——流式布局补项间间距 + 两组间隔
- 用户:「优化程度和排序靠的太近,之间的间隔也没有,看着不好看」;
- 根因:`_FlowLayout` 声明了 `setSpacing(6)` 却只在**换行**时用到(`y += line_height + spacing`),
  水平方向是 `x = next_x` 紧贴上一项 → 项与项之间 0 px;两组之间也没有额外间隔
(离屏实测:「优化程度」下拉框与「排序」标签横向间隔 0 px);
- 解决:① `_do_layout` 水平方向也应用 `spacing`(项与项 6 px,换行判断同步计入);
  ② 两组之间插入 12 px 间隔控件(只在 SMILE 行占位,其它步骤行不受影响);
- 实测(本地与 VM 一致):两组间隔 0 → **24 px**(6+12+6),组内「标签↔下拉框」6 px;
  行宽 280–900 px 时两组始终并排不换行;截图 .codex/visualizations/…/smile_row_spacing.png;
- 测试:新增 tests/test_gui_step_row_layout.py(流式布局项间间距 / 两组间隔 ≥12px /
  间隔控件只在 SMILE 行可见);
...
- 核实:候选评估走 `peak_detection.detect(arr)` 的**默认档**(3σ、仅正峰、不排轴峰),
  本来就没用「峰挑选」步骤的 35σ —— 但 修15 的悬停说明把它写成了「由选峰阈值决定」,
  属**误述**,已更正(下拉说明改为:峰数由 SMILE 自己的低阈值检测 3σ 决定,与选峰步骤无关);
- 同时把评估口径显式化 + 更忠实(避免以后再漂):
  * 新增常量 `SMILE_SCAN_SIGMA = 3.0`(低阈值:宁可多检,不漏弱真峰;伪峰由
    「稳定峰 − 疑伪峰」的排序口径抑制);
  * 新增 `smile_scan_sign_mode(experiment)`:与选峰步骤同源(presets `peak_sign`),
    mixed(HNCACB/CBCANCO 等) → `both`、uniform/未知 → `dominant` —— 原来固定用检测默认的
    仅正峰,会把反相真峰当噪声漏掉;
  * 新增 `smile_scan_edge_margin()`:与选峰步骤同一常量(`pick_peaks._PICK_EDGE_MARGIN=5`),
    评估时排除上下边缘轴峰,避免把未演化信号/边缘伪影算成「稳定真峰」;
  * 新增 `evaluate_candidate_peaks(arr, sign_mode=...)` 统一入口;扫描日志首行打印口径
    (「候选评估口径:阈值 3σ(独立于选峰步骤)、符号模式 …,排除轴峰 5 点」);
- 测试:tests/test_smile_grid_eta.py 增 3 例 —— 低阈值能检出 ~5σ 弱真峰(对照:35σ 会漏)、
  符号模式跟预设(mixed→both / uniform、未知→dominant)、轴峰排除与选峰步骤同源;
  tests/test_gui_smile_row_labels.py 增断言(说明里 3σ/35σ 都写明且不再声称由选峰阈值决定);

## 0.2.199-补29hz-修15(2026-09-11,用户):排序下拉框文案——选项名点明差别 + 悬停说明讲清
- 用户:「排序的悬停文本说明对两个选项的解释不够清楚」(原说明只有一行);
- 选项名改为「净真峰优先(少伪峰)」「一致性优先(残差小)」(下拉里一眼看出差别);
  写入 ui_state / 后端的值不变(`true_peaks` / `consistency`),旧设置兼容;
- 悬停说明重写为 14 行(实测 312 × 196 px,不糊屏):
  * 净真峰优先(默认):按「稳定峰 − 疑伪峰」排 —— 稳定峰 = 多数参数组合都出现的峰、
    疑伪峰 = 只在个别组合出现的峰;适合「尽量少伪峰、尽量多真峰」;峰数由选峰阈值(σ)决定;
  * 一致性优先:按留出采样点残差排 —— 留出一部分已采样点不参与重建,再比较重建值与
    实测值(残差越小越好、相关系数越接近 1 越好),与峰数无关;适合先要谱形/强度可信;
    没有可用留出表时退化为按拟合残差排;
- 测试:新增 tests/test_gui_smile_row_labels.py(选项名点明差别、写值仍是 true_peaks/consistency、
  悬停说明含两种口径的关键信息与量化指标、多行且无超长行);
- 补充(同日,用户问「优化程度的悬停怎么有一个约等于」):组数其实是**精确**的 n×n ——
  实测 2x2=4 组、3x3=9 组、4x4=16 组、5x5=25 组(无重复组合)、默认 4x4=16;原说明里的
  「2x2≈4 组」是多余写法,已改成精确组数并注明「每组各做一次重建+评估」;测试加断言
  (不允许出现「≈/约」、四档精确组数都在、默认 4x4、单行不超长);
- 补充(同日,用户问「选峰阈值(σ)是指」):当时把悬停说明写成「峰的数量由「峰挑选」步骤的
  阈值(σ)决定」——**这是错的**(SMILE 候选评估用的是自己的检测默认档,与选峰步骤的
  35σ 无关),已在 修16 更正为「由 SMILE 自己的低阈值检测(3σ)决定,与「峰挑选」步骤的
  阈值(默认 35σ)无关」。附:选峰阈值(σ)= `noise.estimate` 的 robust MAD σ(1.4826×MAD)
  的倍数,同时用于「强度 > σ×阈值」与「S/N ≥ 阈值」两个条件;

## 0.2.199-补29hz-修14(2026-09-11,用户):下拉框被省略号截断——按文字宽撑开
- 用户报:「优化程度和排序的下拉框内容很多省略号」;
- 根因(离屏实测):系统/QSS 样式下 `QComboBox` 的 `sizeHint` 不含下拉箭头与内边距,
  文本区只剩 2~3 px 余量 —— 「优化程度」=「2x2」19 px 文字 / 22 px 文本区(余 3 px)、
  「排序」=「净真峰优先」60 px / 62 px(余 2 px);字体渲染稍有差异(Windows/DPI/主题)
  Qt 就用省略号截断。同类问题也在数据组面板:「生成谱图」48 px / 50 px(余 2 px);
- 解决:新增 `gui/theme.fit_combo_width(combo, extra=18)` —— 按「最长项文字 + 下拉箭头 +
  余量」设 `minimumWidth`(同时设弹出列表最小宽度),流式布局不会再把它压回去;
  应用到 Pipeline SMILE 行的「优化程度 / 排序」与数据组面板的「处理到 / 依次优化到」;
- 验证:离屏复测四条下拉框文本余量 2~3 px → 16 px;新增 tests/test_gui_combo_width.py
  (守卫「文本区 ≥ 最长项文字 + 8 px」,含 SMILE 行、数据组面板与 helper 本身);
...
- 原实现(修8)在 2D 无 nuslist 时**不判定**、直接假定「密集 + 前 N 个复点」:
  实测两个反例 —— ① 任意子集的密集数据会写成错的前缀表(谱能出、依据不对);
  ② 真稀疏文件(只留采到的行)被当密集跑 → 转换阶段 `nmrPipe -fn MULT` 100% CPU 卡死
(实测 150 s 未结束,残留进程需手动 kill,因为 bruker 声明 -yN 与文件行数不符);
- 新增 `_recover_dense_2d_nus`(替换 `_fid_indirect_points`),判据只用原始数据:
  行数 = ser 字节 / (acqus TD × 4),与「声明网格行数」(2D 复点网格 × 超复数分量)
  比较 —— 相等 = 全格都在(密集)→ 采样点就是零模式里的**非零行**对应的复点索引(支持
  任意子集);小于 = 真稀疏、位置不可知 → 拒绝并提示缺 nuslist;大于 = 元数据与文件不一致
  → 拒绝。ser 缺失时回退用已转换 fid 判定;全格无零行则按满采样表重建(日志注明);
- 判定放在**转换之前**,真稀疏秒级拒绝(不再触发转换卡死);
- VM 真机(全采样 sampleA 造的三份数据,均无 nuslist、NusAMOUNT=25):
  ① 前缀密集(前 64 行非零):成功 8.9 s,恢复 32/128 复点;
  ② **任意子集**密集(随机 32/128 个复点):成功 10.7 s,**恢复出的采样点与真实随机子集完全一致**;
  ③ 真稀疏(64 行 < 网格 256 行):**0.0 s 拒绝**,提示「稀疏文件无法恢复采样位置,请提供 nuslist」;
- 测试:tests/test_2d_nus_compat.py 增 5 例(任意子集精确恢复 / 前缀 / 稀疏拒绝 / 元数据不一致
  拒绝 / 全格无零行),并恢复此前被覆盖掉的 3 个 finalize 用例;
...
- 新增 `rank_mode`:`"true_peaks"`(默认,净真峰=稳定峰−疑伪峰 优先,再稳定峰/平均 S/N/
  留出残差/拟合残差/质量分)与 `"consistency"`(留出采样点残差优先,再拟合残差、净真峰等);
- GUI:Pipeline 的 SMILE 行新增「排序」下拉(净真峰优先 / 一致性优先),按数据写 ui_state
  `smile.rank_mode`,切数据读回;控制器 `optimize_smile(rank_mode=None)` 缺省读该值;
- 测试:新增 `test_rank_modes_differ`(构造「峰多但残差差」vs「峰少但残差好」,断言两种模式
  名次不同);
- 限制(记录):2D 单文件路径 SMILE 用 `-sample None`,没有采样表 → 留出残差不可定义,
  因此 `holdout_*` 目前仅 3D NUS 有效;2D 多文件路径理论上可定义,但 VM 无该类真实数据可验证,
  未实现(不盲写);
- 本地全量 pytest 全绿;VM 全量回归见下;

## 0.2.199-补29hz-修6(2026-09-11,用户):留出采样点数据一致性残差(A 方案落地)
- 用户:按顺序把 A 方案(留出采样点做无参考真值判据)实现到底;
- 映射结论(实物相关性实测,非推算):`nus3d_1`/`nus3d_rc` 的**平面序号 = 直接维点**
  (sampleC = 1202),平面内两轴 = 两个间接维(采样格 20×50 → 重建格 30×75);留出点
  `(k0,k1)` 以 scale=1.0/offset=0 索引平面内 `[k1, k0]`(用 250 个采样点的实测相关性
  在多种轴序/缩放假设里选出,得票最高且用满全部点);
- 实现:`smile_scan(holdout_ratio>0)` 时每组候选重建完成后,取 30 个直接维平面,
  对留出点逐点比较「`nus3d_1` 实测值 vs `nus3d_rc` 重建值」,给出
  `holdout_rmse`(按实测 RMS 归一的中位数)、`holdout_rmse_p90`、`holdout_corr`、
  `holdout_points`、`holdout_planes`;进排序表列,并作为「净真峰 → 稳定峰 → 平均 S/N
  → 留出残差 → 拟合残差 → 质量分」的 tie-break(未启用留出时该值为 0,不影响原行为);
- 训练点拟合残差(修6 前一条):解析 SMILE 每平面 `INITIAL_RMS/FINAL_RMS` 报告,
  取 `FINAL/INITIAL` 中位数 → `smile_rms_ratio`;并修掉「评估回调覆盖已解析指标」的 bug;
  另修「SMILE 每次重写日志(截断)导致第 2 组起 offset 越过 EOF、RMS 缺失」;
- sampleC 真机(sampleC 3D NUS,2x2 + holdout 0.1,4 组 90s):
  nSigma=3/thresh=0.90 → holdout_rmse 0.1141 / corr 0.524;3/0.99 → 0.1131 / 0.518;
  **7/0.90 → 0.0399 / 0.528;7/0.99 → 0.0396 / 0.534** —— 残差能区分参数(高 nSigma
  正则化更强、留出点预测更贴合),750 个留出点(25 点 × 30 平面);
- 文档补账:`577caab`(留出拆分改读原始 nuslist)、`bfe8495`/`29c0c4d`(RMS 指标与合并
  bug)一并记入本条;
...
- 用户方案(2026-09-10):SMILE 优化不必保留候选谱——以终跑脚本为模板**只换
  ...
  间接维」;③ 每跑一份终谱即取指标后**删除**;④ 比较 25 份指标得排序;⑤ 写一份
  参数组合排序表 + 只留前三名脚本。用户选定:方案 B(不自动替换活动谱)、脚本放
  `process/<data_id>_nus_rankN.com`、排序表 CSV+JSON;候选谱放内存盘。
- 本轮(第 1 步,后端骨架,尚未接 GUI/工作流):
  ① `backend/script_generator.split_nus_script()`:把 NUS 终跑脚本切成
     「直接维 → 切片」与「SMILE + 间接维」两段(3D 与 2D 多文件脚本自带切片
     写出/读回,直接切开;2D 单文件脚本 SMILE 用 `-sample None`、改成切片流会改变
     采样语义,故不切,调用方回退整脚本逐组跑);
  ② `rename_nus_scan_output()`:只重命名**最后一行**输出(候选各自独立命名,
     中间产物 nus3d_rc/… 保持不变);
  ③ `NMRPipeBackend.reconstruct_nus(script_only=True)` 只生成脚本不执行;
  ④ `NMRPipeBackend.smile_scan()`:直接维段跑一次 → 每组参数跑 SMILE+间接维 →
     `evaluate(path)` 取指标 → 立即删谱;候选各自命名,不触碰数据 `process/` 下
     的终跑脚本(修正了原实现「25 组共用同一输出名、可能覆盖终跑脚本、Top-N 复制
     到的是最后一组谱」的问题)。
- 测试:`tests/test_smile_scan_script_split.py`(切分点、2D 回退、只改终谱输出名、
  换 SMILE 参数切点不变);相关后端/SMILE 测试全绿。
- 待做(第 2、3 步):工作流 `scan_smile_parameters` + 排序表/前三脚本落盘;
  控制器 `optimize_smile` 改走扫描(内存盘中间目录、不替换活动谱);GUI「按 Rank1
  重跑」入口;文档与 VM 真实 NUS 验证。
## 0.2.199-补29hz-修2(2026-09-10,界面美观性与分区区分度:方案 A)
- 用户:优化界面美观性与各部分的区分度;先检查再改,选定方案 A(保留谱图白底,只做分区与配色统一);
- 检查方法(新增,可复用):离屏渲染主窗口 → `window.grab()` 存图 + 逐点采样像素,用颜色值确认「分区底色 / 分隔条 / 内容区」是否真的分得开(不靠肉眼猜);
- 检查结论(改前):①四个分区的底色其实只有两种——树/Pipeline 内容区 #252526、日志文本框 #1e1e1e,卡片底色被内容控件盖掉;②分隔条 #2d2d30 与内容底 #252526 相差仅 8/255,几乎看不出;③谱图区是纯白(viewer/spectrum_viewer.py setBackground("w"),经典 NMRPipe 观感,本轮按方案 A 保留);
- 改动:
  ① 分区卡片:四个主分区(项目树/CenterPanel/日志/谱图)加 `PanelCard` 外观(底色 #202124 + 1px 边框 #33363b + 6px 圆角)与统一标题栏(项目 / 数据、处理流程、Task / Log、谱图);
  ② 分区内部统一底色:卡片内的 QTreeWidget/QTableWidget/QListWidget/QPlainTextEdit/QTextEdit/QStackedWidget/QScrollArea(含 viewport 与内容控件)一律 #202124,
     Pipeline 滚动区去边框、日志文本框去边框、谱图文件/图层列表不自绘底色 →每个分区是一整块,不再被内容控件切成深浅不一的小块;
  ③ 分隔条提亮:#2d2d30 → #3a3d42(明显可见),悬停 #0e639c、按下 #1177bb;
  ④ 交互态与细节:选中统一 #0e639c/白字、悬停 #2a2d2e、输入框聚焦蓝边、按钮圆角与按下/勾选态、滚动条细身圆角、表头只留下边线、提示气泡圆角内距;
  ⑤ Pipeline 步骤行:底部细线分隔 + 悬停高亮;**状态图标与文字按状态着色**(SUCCESS 绿 / OUTDATED 橙 / FAILED 红 / RUNNING·READY 蓝 / LOCKED 灰,语义色集中在 gui/theme.STATUS_COLORS);
  ⑥ 顶部上下文条改 header 外观(底色 #202124 + 左侧 3px 蓝条 + 更大留白);
- 像素复核(改后,y=300):树/流程/日志均为 #202124,分隔条 #3a3d42,谱图区 #ffffff(保留);面板左上角边框 #33363b 可见;
- 验证:本地全量 pytest 全绿(exit 0);GUI 相关 21 个测试文件单独跑亦全绿;ruff 全仓仍只有 2 处历史告警;VM ~/NMRForge 已同步并回归。
...
- 追加(用户):孤立峰单个罚分上限 20→10(按超出期望间距 5–10 分);
  sampleB 自动谱伪影项由 80 升为 90;QC 相关测试全绿,ruff 通过;

## 0.2.199-补29hn(2026-09-09,选峰默认阈值 25σ → 35σ)
- 用户:选峰默认阈值改为 35;
- 实现:_PICK_THRESHOLD_SIGMA 25→35;pipeline 阈值滑块/步进器默认 250/25.0→350/35.0;
  get_threshold 非 peaks 默认 35;per-data 默认与旧默认 15/25(非自定义)迁移到 35;
- 验证:更新阈值相关测试断言(默认 35.0/350,旧默认迁移到 35);机制类 pick_peaks 测试
  显式 25σ 以与默认解耦;相关测试全绿,ruff 通过;
