...
## 0.2.194(2026-08-26,导入数据下拉恢复独立顶层窗口,修复首次弹出被盖住)

用户反馈:「导入数据」按钮点击不出现下拉内容;点右边的「数据组间分析」
按钮后再点它反而正常出现。

- 根因:0.2.163-补4 为规避 Wayland 把下拉改为主窗口子部件——Windows 上
  子部件首次弹出会被中央部件盖住(z-order 不稳定),点过右边按钮后子部件
  层级变化才正常;之前(0.2.162-补13 起)Windows/X11 一直用非抓取 Tool
  顶层窗口,保证置顶;
- 修复:恢复平台分支——Wayland 用 Popup(xdg_popup,先设 transientParent
  再 show),Windows/X11 用无边框 Tool 顶层窗口(先 show 再 move,
  0.2.162-补14);几何计算改回全局坐标 + 屏幕可用区域;点击外部关闭
  仅对非 Popup(Tool)生效(Wayland 由合成器原生关闭,避免手动 close 与
  dismiss 竞态卡死);导入/数据组间分析两个下拉同一方案;
- 测试:下拉不遮按钮/位置对齐/高度限制改全局坐标断言;全量 pytest 全绿
  + ruff 全绿。
