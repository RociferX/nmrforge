...
## 0.2.199-补29ep(2026-09-01,剔除强峰振铃旁瓣峰)

用户:峰表导入 Poky 看着没问题,还是我软件显示问题——检查为什么还有问题。
排查(VM exp_008/d_001,补29eo 后):强峰标记已对准(偏差 ~0.08px),但
243 个峰中 59 个标记仍「偏离峰顶」3-5px——这些是强峰截断/加窗产生的
振铃旁瓣(真实 3×3 局部极大,但与主峰之间隔着负值谷),被当作独立峰
选中,标记落在旁瓣上;53/59 个确认「与更强峰连线穿过负值」。
修复(core/qc/peak_detection,0.2.199-补29ep):
- 新增 _reject_ringing_sidelobes:候选按强度降序,弱候选若在更强峰
  radius(6px)内且沿连线存在负值(振铃负谷)→ 判为旁瓣剔除;
- 真实重叠峰之间无负谷,保留;
- 测试:+2(旁瓣剔除、重叠峰保留),全量 pytest 843 项全绿。
