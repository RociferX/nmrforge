...
| Check | Result |
| --- | --- |
| Existing `LICENSE` / `COPYING` file | **`LICENSE` (added 2026-09-16): the verbatim Apache License 2.0 text, SPDX `Apache-2.0`.** Since 2026-09-21 the copyright line and the project notice live in `NOTICE` instead, so licence-detection tools see a clean Apache-2.0 file (before the split, GitHub reported the licence as `NOASSERTION`). Before 2026-09-16 the repository was "all rights reserved" by default. |
...
| Copyright holder named in packaging metadata | `NOTICE`: "Copyright 2026 Xuanfeng Li (李宣锋)"; `pyproject.toml` declares the licence SPDX id and `authors = [{ name = "Xuanfeng Li" }]`. |
...
5. State the Qt situation explicitly in `README.md` and `NOTICE`: the project depends on
