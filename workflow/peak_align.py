"""Peak-file whole-spectrum alignment (0.2.199-patch29fw). Move the current peak file as a whole
(all peaks together) in ppm space and search for the offset that maximizes the number of
matching peaks. This is NOT per-peak CSP matching: - row -> {nucleus: ppm}: 2D rows use
H_shift/N_shift/C_shift keys; 3D rows use F1/F2/F3_shift keys plus the nuclei list mapping each
F axis. - Different dimensionalities are handled by using the COMMON nuclei only (e.g. a 2D HSQC
reference vs a 3D HNCA current spectrum is matched on 1H/15N only; the 13C axis does not
participate). - Alignment ratio (user definition, 2026-09-03): matched / min(cur_count,
ref_count). Peaks with real displacement naturally do not match and count as unmatched. -
...
def _best_shift(
    ...
    for i in range(cur.shape[0]):
        d = ref - cur[i]
        # The initial value is normalized by the search range: when the tolerance is very tight (kr
        # 1H +/-0.02), normalizing by the tolerance will bias the "nearest neighbor" towards
        # adjacent peaks (small H difference, large N difference) rather than true pairings.
...
def alignment_figure(
    ...
    *,
    ...
    """Render an alignment check figure (PNG + SVG) of current vs reference peaks. Uses the first
    two common nuclei for the 2D projection (usually 15N/1H); matched pairs are connected by
    ...
    # SVG literal remains text (unified editable SVG convention since 0.2.199).
    ...
    # 0.2.199-patch29fy(user): output the same path at the same time SVG vector version.
